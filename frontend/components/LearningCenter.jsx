...
export default function LearningCenter({ onQuizCorrect }) {
  ...
  const fetchQuizzes = useCallback(async () => {
    ...
    setQuizError(null);
    try {
      const res = await api.get("/api/quiz/fetch?count=8");
      if (res.data.success && res.data.quizzes.length > 0) {
        setQuizItems(res.data.quizzes);
  ...
  const moveToNext = () => {
    if (currentQuizIndex === quizItems.length - 1) return;
          ...
          {quizLoading ? (
            ...
            <>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-2">
                ...
                {showResult && currentQuizIndex === quizItems.length - 1 && (
                  <button
                    onClick={resetQuiz}
