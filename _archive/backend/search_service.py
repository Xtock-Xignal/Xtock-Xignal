...
class SearchEngine:
    def __init__(self):
        disable_in_ci = os.getenv("XT_DISABLE_SEARCH_ENGINE", "").lower() in {"1", "true", "yes", "on"}
        ...
        if disable_in_ci:
            print("Search engine model preload disabled.")
            self.model = None
            self.col_static = None
            self.col_dynamic = None
            self.client_static = None
            self.client_dynamic = None
            return
